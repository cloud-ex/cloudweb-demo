<template>
    <div class="line-switching-box" @mouseover="show = !showGuide" @mouseleave="show = false">
        <div class="name">
            <span>{{currentHost === 'api.lpmex.com'?$t('header.lineSwitch3'):(currentHost === 'api.ginfexapp.top' ? $t('header.lineSwitch2') :$t('header.lineSwitch1'))}}</span>
        ...
        <ul class="list" v-if="!showGuide && show">
            ...
            <li @click="switchLine('api.lpmex.com')">{{$t('header.lineSwitch3')}}</li>
        </ul>
    </div>
</template>
<script>

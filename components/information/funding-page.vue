           ...
           <table>
                 ...
                 <tbody>
                      <tr :key="item.timestamp" v-for="item in list">
                        <td>{{(item.timestamp + deliveryCycle) * 1000 |timeFormat}}</td>
                        <td>{{name}}</td>
                        <td>{{ $t('informationPage.hours', {hours: deliveryCycle / 3600}) }}</td>
                        <td>{{item.rate * 100|retainDecimals({decimal: 4})}}%</td>
                        <td>{{item.rate * 300|retainDecimals({decimal: 4})}}%</td></tr>
                 </tbody>
            </table>
       </div>
    </div>
